...
/* トグル開閉 */
function setupToggle(_ref) {
  var triggerSelector = _ref.triggerSelector,
    groupSelector = _ref.groupSelector,
  ...
  document.querySelectorAll(triggerSelector).forEach(function (trigger) {
    var group = trigger.closest(groupSelector);
    var content = group.querySelector(contentSelector);
    if (!group) return;
    var applyState = function applyState(isOpen) {
